import move
import gamestest as game

def addDirection(pos, direction):
	D = move.directions[direction]
	return (pos[0] + D[0], pos[1] + D[1])

def findMove(grid,marble,symbol):
	moves=[]
	alignement=[]
	possibilities=[]
	for direction in move.directions :
		#print('test diretion :',direction)
		find=True
		e=1        #nombre d'élément de l'alignement
		alignement.clear()
		while find and e<6:           
			alignement.append(checkonBoard(grid,(marble[0]+move.directions[direction][0]*e,marble[1]+move.directions[direction][1]*e)))
			possibilities=getPossibilities(symbol)
			#print(alignement)
			for i,possibilitie in enumerate(possibilities):
					 
				#print("alignement",alignement,"possiblities",possibilitie,possibilitie == alignement) 
				if possibilitie == alignement :		
					m1 = [marble[0],marble[1]]
					m2 = [marble[0]+move.directions[direction][0],marble[1]+move.directions[direction][1]]
					m3 = [marble[0]+move.directions[direction][0]*2,marble[1]+move.directions[direction][1]*2]							
					find=False
										
					if i == 0: 
						#print("0")
						moves.append([[m1],direction])				
					#if i == 1:
						#print("1")
					if i == 2: 
						#print("2")
						moves.append([[m1,m2],direction])
						sumito2m(grid,m1,m2,moves,direction)
					if i==3:
						#print("3")
						sumito2m(grid,m1,m2,moves,direction)
					if i == 4:
						#print("4")
						moves.append([[m1,m2,m3],direction])
						sumito3m(grid,m1,m2,m3,moves,direction)
							
					if i == 5:
						#print("5")
						sumito3m(grid,m1,m2,m3,moves,direction)
					if (i == 6):
						moves.append([[m1,m2],direction])
						sumito2m(grid,m1,m2,moves,direction)
						#print("6")
					if i == 7:
						#print("7")
						moves.append([[m1,m2],direction])
						sumito2m(grid,m1,m2,moves,direction)	
								
					if i == 8:
						#print("8")
						sumito2m(grid,m1,m2,moves,direction)
					if i == 9:
						#print("9")
						sumito3m(grid,m1,m2,m3,moves,direction)
					if i == 10 or i==11:#possible sumito
						#print("10 et 11")
						moves.append([[m1,m2,m3],direction])
						sumito3m(grid,m1,m2,m3,moves,direction)
 
						#print("ok")

					if i == 12 or i==13:
						#print("12 et 13")
						moves.append([[m1,m2,m3],direction])
						sumito3m(grid,m1,m2,m3,moves,direction)
						
					if i == 14:
						#print("14")
						sumito3m(grid,m1,m2,m3,moves,direction)
						pass


					break


			e=e+1   
	return moves

#manque les mouvements en paralleles

			
def checkonBoard(grid,marble):#
	li,ci = marble
	#print(marble)
	if  (0<=li<=4 and 0<=ci<=4) or (5<=li<=8 and 5<=ci<=8) or (5<=li<=8 and 0<=ci<=4 and not grid[li][ci] == 'X') or (0<=li<=4 and 5<=ci<=8 and not grid[li][ci] == 'X'):
		res=grid[marble[0]][marble[1]]

		return res
	else : 
		return 'X'

def isonBoardv2(grid,marble):
	li,ci = marble
	if  (0<=li<=4 and 0<=ci<=4) or (5<=li<=8 and 5<=ci<=8) or (5<=li<=8 and 0<=ci<=4 and not grid[li][ci] == 'X') or (0<=li<=4 and 5<=ci<=8 and not grid[li][ci] == 'X'):
		return True
	else :
		return False 

def getPossibilities(symbol):         
	possibilities =[
					['E'],                 				#:True,    #0    
					['X'],								#:False		#1
					[symbol,'E'],            			 #:True,    #2    possible sumito
					[symbol,'X'],						#False,		#3		possible sumito
					[symbol,symbol,'E'],         			#:True,    #4    possible sumito
					[symbol,symbol,'X'],					#False,		#5	possible sumito
					[symbol,move.opponent(symbol),'E'],        			#:True,    #6    #je sors une bille adverse		possible sumito
					[symbol,move.opponent(symbol),'X'],					#:True 		#7														possible sumito
					[symbol,move.opponent(symbol),move.opponent(symbol)],        					#:False,   #8    #je ne peux pas bouger			possible sumito
					[symbol,symbol,symbol],        					 					#:False,   #9    #je ne peux pas bouger			possible sumito
					[symbol,symbol,move.opponent(symbol),'E'],     							#:True,    #10    #je sors une bille adverse	possible sumito
					[symbol,symbol,move.opponent(symbol),'X'],								#:True, 	#11									possible sumito
					[symbol,symbol,move.opponent(symbol),move.opponent(symbol),'E'], 				#:True,    #12    #je sors une bille adverse	possible sumito
					[symbol,symbol,move.opponent(symbol),move.opponent(symbol),'X'],				#:True 		#13									possible sumito
					[symbol,symbol,move.opponent(symbol),move.opponent(symbol),move.opponent(symbol)], #:False,   #14    #je ne peux pas bouger		possible sumito
					]

	return possibilities

def sumito3m(grid,m1,m2,m3,moves,direction):
	directionlist = []
	for elem in [m1,m2,m3]:#pour chaque position
		for eachdirection in move.directions :#pour chaque direction
			#print(direction)
			if eachdirection == direction or eachdirection == move.opposite[direction]:
				pass
			else:# directions de lalignement exclu
				if isonBoardv2(grid,addDirection(elem, eachdirection)):
					if move.getStatusgrid(grid,addDirection(elem, eachdirection)) == 'E':
						directionlist.append(eachdirection)
						#print(directionlist)
	for i in move.directions:
		if directionlist.count(i) == 3 :
			moves.append([[m1,m2,m3],i])
			pass	
	directionlist.clear()
	return None

def sumito2m(grid,m1,m2,moves,direction):
	directionlist = []
	for elem in [m1,m2]:#pour chaque position
		for eachdirection in move.directions :#pour chaque direction
			#print(direction)
			if eachdirection == direction or eachdirection == move.opposite[direction]:
				pass
			else:# directions de lalignement exclu
				if isonBoardv2(grid,addDirection(elem, eachdirection)):
					if move.getStatusgrid(grid,addDirection(elem, eachdirection)) == 'E':
						directionlist.append(eachdirection)
						#print(directionlist)
	#print("longeur", len(directions))
	for i in move.directions:
		if directionlist.count(i) == 2 :
			moves.append([[m1,m2],i])
			pass	
	directionlist.clear()
	return None

if __name__=='__main__':  
	grid=[
			['W', 'W', 'W', 'W', 'W', 'W', 'X', 'X', 'X'],
			['W', 'W', 'W', 'W', 'W', 'W', 'X', 'X', 'X'],
			['E', 'E', 'W', 'W', 'W', 'E', 'E', 'X', 'X'],
			['E', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'X'],
			['E', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'E'],
			['X', 'E', 'E', 'E', 'E', 'E', 'E', 'E', 'E'],
			['X', 'X', 'E', 'E', 'B', 'B', 'B', 'E', 'E'],
			['X', 'X', 'X', 'B', 'B', 'B', 'B', 'B', 'B'],
			['X', 'X', 'X', 'X', 'B', 'B', 'B', 'B', 'B']
		]
	def show(state):
		print('\n'.join([' '.join(line) for line in state]))
		print()
	
	#grid[4][0] = 'W'
	grid[4][0] = 'B'
	grid[4][1] = 'B'
	grid[4][2] = 'B'
	grid[4][3] = 'B'
	#grid[4][3] = 'B'
	grid[5][3] = 'W'
	#grid[3][2] = 'W'
	#grid[3][1] = 'W'

	result=findMove(grid,(2,2),'W')
	#show(grid)
	#print(result)

			
	

